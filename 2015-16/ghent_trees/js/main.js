	...
	var App = {
		...
		getTreesInventoryFromAPI: function() {
			...
			this._treesInventoryAPI.getTreesInventory().then(
				function(data) {
					var treesInventoryOriginal = data.features;
					...
					for(var i = 0; i < l; i++) {
						...
						if(treeOriginal != null && treeOriginal.properties != undefined) {
							tree = {
								"ghentid": treeOriginal.properties.BoomID,
								"ghentcode": treeOriginal.properties.Boomcode,
								"name": treeOriginal.properties.Naam,
								"namenl": treeOriginal.properties.NaamNL,
								"dimensions": {
									"height": treeOriginal.properties.Boomhoogte,
									"diameter": treeOriginal.properties.Stamdiameter
								},
								"dayofbirth": parseInt(treeOriginal.properties.Plantjaar),
								"mutationdate": new Date(treeOriginal.properties.Mutatiedatum),
								"geolocation": {
									"lat": parseFloat(treeOriginal.geometry.coordinates[0]),
									"lng": parseFloat(treeOriginal.geometry.coordinates[1])
								}
							}; 
